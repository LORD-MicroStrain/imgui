...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const float label_pad_width = (label_size.x > 0.0f) ? (label_size.x + style.ItemInnerSpacing.x) : 0.0f;
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_pad_width + w, label_size.y + style.FramePadding.y * 2.0f));
    const ImRect bb(window->DC.CursorPos + ImVec2(label_pad_width, 0.0f), total_bb.Max);
    ItemSize(total_bb, style.FramePadding.y);
    ...
    // Render label
    if (label_size.x > 0)
        RenderText(total_bb.Min + ImVec2(0.0f, style.FramePadding.y), label);
